...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train_set, batch_size=BATCH_SIZE, shuffle=True, pin_memory=True, collate_fn=collate_fn, num_workers=cpu_count(), drop_last=True)
    test_loader = DataLoader(test_set, batch_size=BATCH_SIZE, shuffle=False, pin_memory=True, collate_fn=collate_fn, num_workers=cpu_count(), drop_last=True)

    print('init model')
    criterion = nn.CrossEntropyLoss()
    model, optimizer = transfomer.init_model(dictionary, device)
    ...
    for epoch in range(N_EPOCHS):
        ...
        print('\n')
        # write to log
        utils.write_log(log, 'tf_mix_noctx_full.txt')
        # save checkpoint
        utils.save_model(model, optimizer, {}, 'models/tf_mix_noctx_full.pt')
